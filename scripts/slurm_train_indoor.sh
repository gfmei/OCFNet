...
# `sacctmgr modify user $USER set DefaultAccount=<account>`. The requeue below re-submits this
# same file and forwards the running job's account, so a chain started with an explicit
# --account keeps it.
#
# Do not edit this file while a job is running it: bash reads a script incrementally, so
# changing its length can derail the running shell. A requeue chain was lost this way.
