...
def remap_legacy_state_dict(state_dict):
    """Rename parameters saved before the overlap-attention module was renamed.

    Checkpoints written while the module was still called a graph network carry `gnn.*` and
    `proj_gnn.*`. Drop this once no such checkpoint is in use.
    """
    legacy = {'gnn.': 'attention.', 'proj_gnn.': 'proj_feats.'}
    return {next((new + key[len(old):] for old, new in legacy.items()
                  if key.startswith(old)), key): value
