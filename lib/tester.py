from lib.trainer import Trainer, PredatorTrainer, OCFNetTrainer
import os, torch
...
class PredatorTester(PredatorTrainer):
    """3DMatch/3DLoMatch tester for Predator: dumps per-point descriptors and scores.

    Separate from `OCFNetTester` because the two models emit different things -- Predator
    per-point descriptors that the benchmark matches itself, OCFNet finished
    correspondences read off its transport plan.
    """
    def __init__(self,args):
        PredatorTrainer.__init__(self,args)
    ...
    def test(self):
        ...
        with torch.no_grad():
            for idx in tqdm(range(num_iter)): # loop through this epoch
                ...
                # the collate stacks one transform per pair; the benchmark runs pair by pair
                c_rot = input_dict['rot'][0].to(self.device)
                c_trans = input_dict['trans'][0].to(self.device)
...
class OCFNetTester(OCFNetTrainer):
    """Dump the correspondences OCFNet predicts for every benchmark pair.

    Unlike Predator, the model outputs correspondences rather than descriptors: the coarse
    stage proposes super-point matches (Eq. 3), the patches around them are matched by
    optimal transport and read out row-wise (Eq. 6). What lands on disk is therefore a list
    of point pairs and their confidences, which scripts/evaluate_ocfnet.py feeds to RANSAC.
    """
    ...
    def test(self):
        ...
        with torch.no_grad():
            for idx in tqdm(range(num_iter)):
                ...
                data = dict()
                data['src_pcd'] = input_dict['pcd_src'].cpu()
                data['tgt_pcd'] = input_dict['pcd_tgt'].cpu()
                data['correspondences'] = matches.cpu()
                data['scores'] = scores.cpu()
                data['n_coarse'] = int(output['coarse_matches'].shape[0])
                # how many of the predicted patch pairs really overlap
                from models.ocfnet import ground_truth_coarse_pairs
                truth = ground_truth_coarse_pairs(
                    input_dict['correspondences'].long().to(self.device),
                    output['src_patch_id'], output['tgt_patch_id'])
                predicted = output['coarse_matches']
                if predicted.shape[0] and truth.shape[0]:
                    width = int(output['tgt_patch_id'].max()) + 1
                    data['coarse_inlier_ratio'] = float(torch.isin(
                        predicted[:, 0] * width + predicted[:, 1],
                        truth[:, 0] * width + truth[:, 1]).float().mean())
                # the collate stacks one transform per pair; the benchmark runs pair by pair
                data['rot'] = input_dict['rot'][0].cpu()
                data['trans'] = input_dict['trans'][0].cpu()

                torch.save(data, f'{self.snapshot_dir}/{self.config.benchmark}/{idx}.pth')


class KITTITester(PredatorTrainer):
    """
    KITTI tester
    """
    def __init__(self,args):
    ...
    def test(self):
        ...
        with torch.no_grad():
            for _ in tqdm(range(num_iter)): # loop through this epoch
                ...
                # forward pass
                sinput_src = make_sparse_tensor(
                    inputs['src_C'], inputs['src_F'], device=self.device)
                sinput_tgt = make_sparse_tensor(
                    inputs['tgt_C'], inputs['tgt_F'], device=self.device)
                ...
                len_src = inputs['len_batch'][0][0]
                c_rot, c_trans = inputs['rot'][0], inputs['trans'][0]
...
class ModelnetTester(PredatorTrainer):
    """
    Modelnet tester
    """
    def __init__(self,args):
...
def get_trainer(config):
    # config.model holds the instantiated network by the time main.py gets here
    if type(config.model).__name__ == 'OCFNet':
        return OCFNetTester(config) if config.mode == 'test' else OCFNetTrainer(config)
    if(config.dataset == 'indoor'):
        # train mode must return the trainer: returning a tester here meant a Predator
        # training run was driven by an object whose job is to dump features
        return PredatorTester(config) if config.mode == 'test' else PredatorTrainer(config)
