...
class MetricLoss(nn.Module):
    ...
    def forward(self, src_pcd, tgt_pcd, src_feats, tgt_feats, correspondence, rot, trans,
                scores_overlap, scores_saliency, scale, len_batch=None):
        """Loss of a batch of point cloud pairs, averaged over the pairs.

        Every tensor holds the pairs of the batch concatenated, in the order the collate
        function put them (rows of the backbone output follow the input order), so
        `len_batch = [[n_src, n_tgt], ...]` is enough to cut them apart. `rot`/`trans` may be
        stacked per pair ([B, 3, 3] / [B, 3, 1]) and `scale` may be a sequence.

        Without `len_batch` the input is treated as a single pair, exactly as upstream.
        """
        if len_batch is None:
            return self.forward_single(src_pcd, tgt_pcd, src_feats, tgt_feats, correspondence,
                                       rot, trans, scores_overlap, scores_saliency, scale)

        n_src_total = src_pcd.size(0)
        src_start, tgt_start, all_stats = 0, 0, []
        for b, (n_src, n_tgt) in enumerate(len_batch):
            # correspondences carry the batch offsets of the collate, make them pair-local
            sel = (correspondence[:, 0] >= src_start) & (correspondence[:, 0] < src_start + n_src)
            corr = correspondence[sel] - correspondence.new_tensor([src_start, tgt_start])

            all_stats.append(self.forward_single(
                src_pcd[src_start:src_start + n_src], tgt_pcd[tgt_start:tgt_start + n_tgt],
                src_feats[src_start:src_start + n_src], tgt_feats[tgt_start:tgt_start + n_tgt],
                corr,
                rot[b] if rot.dim() == 3 else rot,
                trans[b] if trans.dim() == 3 else trans,
                torch.cat([scores_overlap[src_start:src_start + n_src],
                           scores_overlap[n_src_total + tgt_start:n_src_total + tgt_start + n_tgt]]),
                torch.cat([scores_saliency[src_start:src_start + n_src],
                           scores_saliency[n_src_total + tgt_start:n_src_total + tgt_start + n_tgt]]),
                scale[b] if hasattr(scale, '__len__') else scale))
            src_start, tgt_start = src_start + n_src, tgt_start + n_tgt

        return {key: sum(stats[key] for stats in all_stats) / len(all_stats)
                for key in all_stats[0]}

    def forward_single(self, src_pcd, tgt_pcd, src_feats, tgt_feats, correspondence, rot, trans,scores_overlap,scores_saliency, scale):
        """
