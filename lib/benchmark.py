...
def computeTransformationErr(trans, info):
    ...
    t = trans[:3, 3]
    r = trans[:3, :3]
    # scipy returns (x, y, z, w); nibabel.quaternions.mat2quat used to return (w, x, y, z)
    # with a non-negative w, and the quadratic form below is sensitive to that sign
    q = Rotation.from_matrix(r).as_quat()
    if q[3] < 0:
        q = -q
    er = np.concatenate([t, q[:3]], axis=0)
...
def benchmark(est_folder,gt_folder):
    ...
    with open(f'{est_folder}/result','w') as f:
        ...
        for idx,scene in enumerate(scene_names):
            ...
            # a scene where nothing registered successfully leaves these empty; report nan
            # instead of crashing, which is what an undertrained model runs into
            for stats, errors in ((re_per_scene, re), (te_per_scene, te)):
                for key, reduce in (('mean', np.mean), ('median', np.median),
                                    ('min', np.min), ('max', np.max)):
                    stats[key].append(reduce(errors) if errors.size else float('nan'))
        ...
        # nanmean: scenes where nothing registered contribute no rotation/translation error
        f.write("Mean median RRE: {:.3f}: +- {:.3f}\n".format(np.nanmean(re_per_scene['median']), np.nanstd(re_per_scene['median'])))
        f.write("Mean median RTE: {:.3F}: +- {:.3f}\n".format(np.nanmean(te_per_scene['median']),np.nanstd(te_per_scene['median'])))
    f.close()
