...
try:
    import coloredlogs
except ImportError:  # optional, only makes the console output prettier
    coloredlogs = None
try:
    import git
except ImportError:  # optional, only used to log the commit the run started from
    git = None
...
def print_info(opt, log_dir=None):
    ...
    # Print commit ID
    if git is None:
        _logger.info('GitPython is not installed, skipping the commit info')
        return
    try:
