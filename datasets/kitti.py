...
from lib.benchmark_utils import get_registration_module
    ...
    def __getitem__(self, idx):
        ...
        if key not in self.kitti_icp_cache:
            if not os.path.exists(filename):
                ...
                reg = registration.registration_icp(pcd0, pcd1, 0.2, np.eye(4),
                                                        registration.TransformationEstimationPointToPoint(),
                                                        registration.ICPConvergenceCriteria(max_iteration=200))
        ...
        _, sel_src = sparse_quantize(np.ascontiguousarray(src_pcd_input) / self.voxel_size, return_index=True)
        _, sel_tgt = sparse_quantize(np.ascontiguousarray(tgt_pcd_input) / self.voxel_size, return_index=True)

        # get correspondence
        tsfm = to_tsfm(rot, trans)
        src_xyz, tgt_xyz = src_pcd_input[sel_src], tgt_pcd_input[sel_tgt] # raw point clouds
        matching_inds = get_correspondences(src_xyz, tgt_xyz, tsfm, self.search_voxel_size * scale)
