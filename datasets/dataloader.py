...
from lib.spconv_utils import sparse_collate
...
def collate_pair_fn(list_data):
    ...
    # prepare inputs for the sparse backbone
    src_batch_C, src_batch_F = sparse_collate(src_coords, src_feats)
    tgt_batch_C, tgt_batch_F = sparse_collate(tgt_coords, tgt_feats)
    ...
    return {
        ...
        'len_batch': len_batch,
        # one entry per pair: with batch_size > 1 the loss needs every pair's own transform
        'rot': torch.stack([to_tensor(r) for r in rot]),
        'trans': torch.stack([to_tensor(t) for t in trans]),
